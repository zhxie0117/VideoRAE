...
class LARPTokenizerTrainer(BaseTrainer):
    def __init__(self, rank, cfg):
        ...
        self.loss_repa_weight = float(cfg.get('loss_repa_weight', 0.2))
        self.loss_action_weight = float(cfg.get('loss_action_weight', 0.05))
    ...
    def _iter_step(self, data, is_train):
        start = time.time()
        teacher_video = data['gt_teacher'].to(self.device) if 'gt_teacher' in data else None
        video = data['gt'].to(self.device)
        ...
        try:
            ...
            sqt_weight = 0
            #self.log(f'Failed to set stochastic temperature for bottleneck regularizer: {e}')
        ...
        with torch.autocast(device_type='cuda', dtype=self.amp_dtype, enabled=self.use_amp):
            ...
            if not is_train: # only calculate FVD and/or FID for validation
                if pred_frames.shape[2] >= 10: # FVD calc requires at least 10 frames
                    ...
                    self.running_real_stats = self.fvd_calculator.get_feature_stats_for_batch(video, self.running_real_stats)
                    ...
                    self.img_running_real_stats = self.fid_calculator.get_feature_stats_for_batch(video, self.img_running_real_stats)
        ...
        with torch.autocast(device_type='cuda', dtype=self.amp_dtype, enabled=self.use_amp):
            ...
            mses = ((pred_frames - video)**2).reshape(B, -1).mean(dim=-1)
            psnr = -10 * torch.log10(mses).mean()
            ssim_v = ssim(pred_frames.flatten(end_dim=1), video.flatten(end_dim=1), data_range=1)
            ...
            if 'repa_loss' in model_output:
                repa_loss = model_output.pop('repa_loss')
                loss = loss + repa_loss * self.loss_repa_weight
                info_dict['repa_loss'] = repa_loss.item()
            if 'align_loss' in model_output:
                align_loss = model_output.pop('align_loss')
                loss = loss + align_loss * self.loss_repa_weight
                info_dict['align_loss'] = align_loss.item()

            if 'loss_action' in model_output:
                loss_action = model_output.pop('loss_action')
                if self.loss_action_weight > 0:
                    loss = loss + loss_action * self.loss_action_weight
                info_dict['loss_action'] = loss_action.item()


            if 'loss_quant' in model_output:
                loss_quant = model_output.pop('loss_quant')
                loss = loss + loss_quant * self.get_loss_q_weight()
                info_dict['loss_quant'] = loss_quant.item()
            ...
            if 'bottleneck_rep' in model_output:
                with torch.no_grad():
    ...
    def _gen_vis_result(self, tag, vislist):
        print(f'Generating visualization results for {tag}...')
        # self.model_ddp.eval()
